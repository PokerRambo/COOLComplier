...
void CgenClassTable::code_class_nameTab(){
  ...
  for (int i = 0; i < class_nodes.size(); i++){
    CgenNode* class_node = class_nodes[i];
...
void CgenClassTable::code_dispatchTabs(){
  ...
  for(int i = 0; i < class_nodes.size(); i++){
    CgenNode* _class_node = class_nodes[i];
    ...
    for (int mi = 0; mi < full_methods.size(); mi++) {
        method_class* _method = full_methods[mi];
...
std::vector<method_class*> CgenNode::GetFullMethods() {
    if (m_full_methods.empty()) {
        ...
        for (int ci = 0; ci < inheritance.size(); ci++) {
            CgenNode* _class_node = inheritance[ci];
            Symbol _class_name = _class_node->name;
            std::vector<method_class*> _methods = _class_node->GetMethods();
            for (int mi = 0; mi < _methods.size(); mi++) {
                method_class* _method = _methods[mi];
                Symbol _method_name = _method->name;
...
std::vector<attr_class*> CgenNode::GetFullAttribs() {
    if (m_full_attribs.empty()) {
        ...
        for (int ci = 0; ci < inheritance.size(); ci++) {
          CgenNode* class_node = inheritance[ci];
...
void CgenNode::code_methods(ostream& s) {
    ...
    for (int mi = 0; mi < methods.size(); mi++) {
        method_class* method = methods[mi];
        method->code(s, this);
    }
}
...
void CgenClassTable::code_protObjs() {
    std::vector<CgenNode*> class_nodes = GetClassNodes();
    for (int ci = 0; ci < class_nodes.size(); ci++) {
        CgenNode* class_node = class_nodes[ci];
...
void static_dispatch_class::code(ostream &s, Environment env) {
    ...
    for (int ei = 0; ei < actuals.size(); ei++) {
        Expression expr = actuals[ei];
        expr->code(s, env);
        emit_push(ACC, s);
        env.EnterScope();
        env.AddObstacle();
...
// two helper function: to generate all children tags for cases.
// one level at a time, in order to find least ancestor branch.

bool HasFinished(std::vector<std::vector<int> > __cases_tags) {
    for (int i = 0; i < __cases_tags.size(); i++) {
        std::vector<int> __case_tags = __cases_tags[i];
...
std::vector<int> GetChildrenTagsSet(std::vector<int> __curr_tags, std::map<Symbol, int> _class_tags, std::vector<CgenNode*> _class_nodes) {
    std::vector<int> __children_tags; // for return.
    for (int ci = 0; ci < __curr_tags.size(); ci++) { // find children of this class.
        int __curr_tag = __curr_tags[ci];
        CgenNode* __curr_node = _class_nodes[__curr_tag];
        std::vector<CgenNode*> __children_nodes = __curr_node->GetChildren();
        for (int cj = 0; cj < __children_nodes.size(); cj++) {
            CgenNode* __children_node = __children_nodes[cj];
...
void typcase_class::code(ostream &s, Environment env) {
    ...
    for (int bi = 0; bi < _cases.size(); bi++) {
        branch_class* _case = _cases[bi];
        Symbol _type_decl = _case->type_decl;
        int _class_tag = _class_tags[_type_decl];
        //std::vector<int> case_tags = { _class_tag }; //should be changed
    ...
    while (!HasFinished(cases_tags)) {
        ...
        for (int caseidx = 0; caseidx < cases_tags.size(); ++caseidx) {
            ...
            for (int cti = 0; cti < case_tags.size(); cti++) {
                int case_tag = case_tags[cti];
        ...
        for (int i = 0; i < cases_tags.size(); ++i) {
            cases_tags[i] = GetChildrenTagsSet(cases_tags[i], _class_tags, _class_nodes);
        }
    }
